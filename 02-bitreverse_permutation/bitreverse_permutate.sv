...
module bitreverse_permute #(
    ...
    logic [MAX_BLOCK_LENGTH_LOG2:0] rev_addr;
    ...
    always_ff @(posedge clk_i) begin
        if (srst_i) begin
            ...
            if (block_ready[read_bank]) begin
                ...
                rev_addr = bitrev(read_cnt, block_length_log2_i + 1);

                if (read_bank == 0) tdata_o <= mem0[rev_addr];
                else tdata_o <= mem1[rev_addr];
